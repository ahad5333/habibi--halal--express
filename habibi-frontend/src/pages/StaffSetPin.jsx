...
export default function StaffSetPin() {
  ...
  const handleNext = async () => {
    ...
    try {
      const res  = await fetch(`${API}/api/staff/set-pin`, {
        ...
        body: JSON.stringify({ staff_id: staffId, pin, confirm_pin: confirm }),
      });
  ...
  if (!staffId || !token) return (
    <div style={s.page}><div style={s.card}>
      <p style={{ color: '#f87171', marginBottom: '1rem' }}>Invalid setup link. Please ask your manager to resend it.</p>
      <Link to="/staff/login" style={{ color: '#E5B64E', fontSize: '0.9rem' }}>Go to Staff Login →</Link>
    </div></div>
  );
  ...
  if (step === 'done') return (
    <div style={s.page}><div style={s.card}>
      ...
      <Link to="/staff/login" style={{ display: 'block', background: '#E5B64E', color: '#0a0a0a', borderRadius: 10, padding: '0.9rem', fontWeight: 700, fontSize: '1rem', textDecoration: 'none' }}>
        Go to Staff Login →
      </Link>
    </div></div>
  );
