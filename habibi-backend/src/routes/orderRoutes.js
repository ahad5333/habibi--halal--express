...
router.post("/guest",
  ...
  body('customer_phone').optional({ checkFalsy: true }).trim().isLength({ max: 20 }).withMessage('Phone too long.')
  ...
  body('payment_reference').optional({ checkFalsy: true }).trim().isLength({ max: 100 }).withMessage('Payment confirmation number is too long.'),
